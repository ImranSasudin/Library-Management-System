       			 ...
       			 <div id="container" class="container">
          			  <div class="row text-black">
               			 <div class="col-lg-8 offset-sm-2 text-center">
                 			   <h1 class="display-5">Returned Books</h1>
                 			   <div class="text-left">
                 			   </div>
                   				 <div class="info-form">
                          			  ...
                          			  <table id="dtBasicExample" class="table table-striped table-bordered" cellspacing="0" width="100%">
  										...
  										<tbody>
  											<c:forEach items="${reports}" var="report">
    											<tr>
    												<td><a href="/Library/ReportController?action=viewBook&report=returnedBooks&bookISBN=${report.bookISBN}" class="text-primary"><c:out value="${report.bookISBN}" /></a></td>
													<td><c:out value="${report.bookName}" /></td>
													<td><c:out value="${report.returnDate}" /></td>
													<td><c:out value="RM${report.paidFine}" /></td>
													<c:forEach items="${students}" var="student">
														<c:choose>
															<c:when test="${report.patronID == student.patronID}">
															<td><a href="/Library/ReportController?action=viewPatron&type=Student&report=returnedBooks&patronID=${report.patronID}" class="text-primary"><c:out value="${report.patronName}" />  [Student]</a></td>
															</c:when>
														</c:choose>
													</c:forEach>
													<c:forEach items="${teachers}" var="teacher">
														<c:choose>
															...
															<td><a href="/Library/ReportController?action=viewPatron&type=Teacher&report=returnedBooks&patronID=${report.patronID}" class="text-primary"><c:out value="${report.patronName}" />  [Teacher]</a></td>
															</c:when>
														</c:choose>
													</c:forEach>
												</tr>
